...
export default {
	...
	async mounted() {
		try {
			const response = await axios.get(generateUrl('/apps/nostcloud/notes'))
	...
	methods: {
		...
		async createNote(note) {
			this.updating = true
			try {
				const response = await axios.post(generateUrl('/apps/nostcloud/notes'), note)
		...
		async updateNote(note) {
			this.updating = true
			try {
				await axios.put(generateUrl(`/apps/nostcloud/notes/${note.id}`), note)
			} catch (e) {
				console.error(e)
		...
		async deleteNote(note) {
			try {
				await axios.delete(generateUrl(`/apps/nostcloud/notes/${note.id}`))
				this.notes.splice(this.notes.indexOf(note), 1)
				if (this.currentNoteId === note.id) {
					this.currentNoteId = null
				}
				showSuccess(t('nostcloud', 'Note deleted'))
			} catch (e) {
				console.error(e)
				showError(t('nostcloud', 'Could not delete the note'))
			}
		},
	},
}
