<template>
    <NcAppNavigation>
        <template #list>
            <NcAppNavigationNewItem :title="t('nostcloud', 'Create note')" @new-item="$emit('create-note', $event)">
                <template #icon>
                    <PlusIcon />
                </template>
            </NcAppNavigationNewItem>
            <h2 v-if="loading" class="icon-loading-small loading-icon" />
            <NcEmptyContent v-else-if="sortedNotes.length === 0" :title="t('nostcloud', 'No notes yet')">
                <template #icon>
                    <NoteIcon :size="20" />
                </template>
            </NcEmptyContent>
            <NcAppNavigationItem v-for="note in sortedNotes" :key="note.id" :name="note.name"
                :class="{ selectedNote: note.id === selectedNoteId }" :force-display-actions="true" :force-menu="false"
